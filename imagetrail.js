/**
 * ImageTrail Component (Vanilla JS implementation based on React Bits)
 * Powered by GSAP
 */
...
class ImageTrailVariant7 {
  ...
  showNextImage() {
    ...
    gsap.killTweensOf(img.DOM.el);
    const scaleValue = gsap.utils.random(0.5, 1.4);
...
class ImageTrailVariant8 {
  ...
  showNextImage() {
    ...
    this.rotation.x = -(relY / (centerY || 1)) * 30;
    this.rotation.y = (relX / (centerX || 1)) * 30;
    this.cachedRotation = { ...this.rotation };

    const distanceFromCenter = Math.sqrt(relX * relX + relY * relY);
    const maxDistance = Math.sqrt(centerX * centerX + centerY * centerY);
    const proportion = distanceFromCenter / (maxDistance || 1);
...
/**
 * Controller function for initializing ImageTrail in Vanilla JS
 */
function initImageTrail(containerElement, items = [], defaultVariant = 1) {
  ...
  function setVariant(variantNum) {
    if (currentInstance && typeof currentInstance.destroy === 'function') {
      currentInstance.destroy();
    }
    // Reset image styles
    const imgs = containerElement.querySelectorAll('.content__img');
    imgs.forEach(img => {
      gsap.killTweensOf(img);
      gsap.set(img, { opacity: 0, scale: 1, x: 0, y: 0, rotation: 0, filter: 'none' });
    });

    currentVariant = variantNum;
    const VariantClass = variantMap[variantNum] || variantMap[1];
    currentInstance = new VariantClass(containerElement);
  }
...
window.initImageTrail = initImageTrail;
